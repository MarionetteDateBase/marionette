...
public class AES implements BlockCipher {
    ...
    /**
     * 这个函数看上去其貌不扬，但其实是整个AES加密的核心，请不要忽略！
     * 这个函数是用来求入参x的2倍，但是重点来了，我们是在伽罗华域上求解，所以
     * 必须先确定一个GF上的8次不可约多项式。As we all know，Rijndael密码学中，
     * 这个多项式确定为x^8+x^4+x^3+x+1，如果最高位是1的话，继续左移一位的同
     * 时会超出我们指定的伽罗华域，所以要除以同余式，也就是异或0x1B。
     * ps. 以上理论参考《密码编码学与网络安全——原理与实践》（第五版）
     */
